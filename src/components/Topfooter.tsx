...
const Topfooter = () => {
  return (
    <div className="border-t border-b grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
      {topFooterData?.map((data, index) => (
        <div
          key={index}
          className="flex items-center gap-3 p-4 group hover:bg-gray-50 hoverEffect transition-colors"
        >
          {data.icon}
