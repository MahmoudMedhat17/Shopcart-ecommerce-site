    ...
    return (
       <>
        <div className="py-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 border-b border-gray-400">
                ...
                <div className="space-y-3">
                    {
                        quickLinksData?.map((data) => (
                            <div key={data.title}>
                                <Link href={data.href} className="text-gray-600 hover:text-shop-dark-green hoverEffect font-medium mt-4">
                                    {data.title}
                                </Link>
                            </div>
                        ))
                ...
                <div className="space-y-3">
                    {
                        categoriesData?.map((data) => (
                            <div key={data.title}>
                                <Link href={`/category/${data.href}`} className="text-gray-600 hover:text-shop-dark-green hoverEffect font-medium mt-4">
                                    {data.title}
                                </Link>
                            </div>
                        ))
            ...
            <div className="space-y-3">
                ...
                <form action="" className="flex flex-col gap-3">
                    <Input className="max-w-60" placeholder="Enter your email" type="email" required/>
        ...
        <div className="my-4 text-center">
                © {yearDate} <Logo className="text-darkColor hover:text-shop-dark-green hoverEffect"/> . All rights reserved.
        </div>
       </>
    )
};
