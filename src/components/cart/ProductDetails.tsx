    ...
    <div className="flex flex-col gap-8 border-0">
      ...
      {cart.map((item) => {
        ...
        const handleRemoveQuantity = () => {
          if (getItemCount(item.product._id) === 1) {
            deleteProduct(item.product._id);
            toast.success(
              `${item.product.name?.substring(0, 12)} is removed from cart!`,
            );
            return;
          } else {
            removeQuantity(item.product._id);
            toast.success(
              `${item.product.name?.substring(0, 12)} quantity is removed!`,
            );
          }
        };
        ...
        const handleAddQuantity = () => {
          ...
          toast.success(
            `${item.product.name?.substring(0, 12)} quantity is added!`,
          );
        };
        ...
        return (
          ...
          >
            ...
            <Image
              src={
                item.product.images ? urlFor(item.product.images[0]).url() : ""
              }
            ...
            <div className="flex justify-between items-start flex-1">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-8">
                  <div className="flex items-center gap-4">
                    ...
                    <span>{getItemCount(item.product._id)}</span>
                    <span
              ...
              <div className="flex flex-col items-start gap-6">
                <div>
                  ...
                  <p className="text-shopLighterText">per item</p>
                </div>
                <p className="font-semibold text-start">
                  ${getPrice(item.product._id)}
                </p>
              </div>
            </div>
          </div>
